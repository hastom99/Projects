geocodificadorModulo = (function () {
 ...
 geocodificador.geocode({'address': direccion},function(results,status)
  {
      if (status == 'OK') {
    funcionALlamar(direccion,results[0]);
    }
  });
 
  }

  function geodificacionInversa(latLng,funcionALlamar)
  {
    geocodificador.geocode({'location': latLng}, function(results, status) {
      if (status == 'OK') {
        if (results[1]) {
          funcionALlamar(results[0].formatted_address,results[0]);
        }
      }
    });
  }
...
/*var contexto =  this;
    geocodificador.geocode( { 'address': direccion}, function(results, status) {
      if (status == 'OK') {
      funcionALlamar(direccion,results);
  ...
  function inicializar () {
    ...
    document.querySelector('#direccion').addEventListener('keypress', function (e) {
      ...
      if (key === 13) { // 13 is enter
        ...
        self.usaDireccion(direccion, direccionesModulo.agregarDireccionYMostrarEnMapa);
