(function() {
    ...
    function ControladorInformacionDePelicula() {
        this.cargarDetalle = function(data) {
                ...
                $(".imagen").attr("src", pelicula[0].poster);
                $(".titulo, title").html(pelicula[0].titulo + " (" + pelicula[0].anio + ")");
                $(".trama").html(pelicula[0].trama);
                var fecha = new Date(pelicula[0].fecha_lanzamiento);
                $(".lanzamiento").html(fecha.getDate() + "/" + (fecha.getMonth() + 1) + "/" + fecha.getUTCFullYear());
                $(".genero").html(genero);
                $(".director").html(pelicula[0].director);
                $(".duracion").html(pelicula[0].duracion);
                $(".rank").html(pelicula[0].puntuacion + "/10");
                var actores_string = '';
